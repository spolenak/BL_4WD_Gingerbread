...
public class ActivityMCU  extends Activity{
    ...
    private final Handler mHandler =  new Handler() {
        public void handleMessage(android.os.Message msg) {
            ...
            case cBluetooth.RECIEVE_MESSAGE:													// if message is recieved
            	...
            	int FDataLineIndex = sb.indexOf("FData:");					// строка с Flash данными (чтение)
            	int FWOKLineIndex = sb.indexOf("FWOK");						// строка с сообщением об успешной записи в Flash
            	int endOfLineIndex = sb.indexOf("\r\n");

            	if (FDataLineIndex >= 0 && endOfLineIndex > 0 && endOfLineIndex > FDataLineIndex) { 					// если обнаружили
            		String sbprint = sb.substring("FData:".length(), endOfLineIndex);	// вырезаем
            		...
            		sb.delete(0, sb.length());		// очищаем
                }
            	else if (FWOKLineIndex >= 0 && endOfLineIndex > 0 && endOfLineIndex > FWOKLineIndex) {
            		String flash_success = getString(R.string.flash_success);
            		Toast.makeText(getBaseContext(), flash_success, Toast.LENGTH_SHORT).show();
            		sb.delete(0, sb.length());
            	}
            	else if(endOfLineIndex > 0) {
            		String error_get_data = getString(R.string.error_get_data);
            		Toast.makeText(getBaseContext(), error_get_data, Toast.LENGTH_SHORT).show();
            		sb.delete(0, sb.length());
            	}
            	break;    
            }
    ...
    private void loadPref(){
    	...
    	address = mySharedPreferences.getString("pref_MAC_address", address);			// ѕервый раз загружаем дефолтное значение
	}
