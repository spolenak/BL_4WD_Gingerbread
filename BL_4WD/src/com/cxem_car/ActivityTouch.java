...
public class ActivityTouch extends Activity {
    ...
    private int xRperc;				// точка разворота
	...
	private String CalcMotor(float calc_x, float calc_y){
		...
		int xR = Math.round(BIG_CIRCLE_SIZE*xRperc/100);		// Вычисляем значение точки разворота
        ...
        if(motorLeft > 0) {			// Если наклон вперед
        	directionL = "-";
        }      
        if(motorRight > 0) {		// Если наклон вперед
        	directionR = "-";
        }
