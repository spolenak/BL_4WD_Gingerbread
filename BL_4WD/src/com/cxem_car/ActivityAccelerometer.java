...
public class ActivityAccelerometer extends Activity implements SensorEventListener  {
    ...
    private String address;			// MAC-адрес устройства
    private boolean show_Debug;		// отображение отладочной информации
    private int xMax;		    	// предел по оси X, максимальное значение для ШИМ (0-10), чем больше, тем больше нужно наклонять Android-устройство
    private int yMax;		    	// предел по оси Y, максимальное значение для ШИМ (0-10)
    private int yThreshold;  		// минимальное значение ШИМ (порог ниже которого не вращается двигатель)
    ...
    public void onSensorChanged(SensorEvent e) {
        ...
        else if(xAxis < -pwmMax) xAxis = -pwmMax;		// отриц. значение - наклон вправо
        
        if(yAxis > pwmMax) yAxis = pwmMax;
        else if(yAxis < -pwmMax) yAxis = -pwmMax;		// отриц. значение - наклон вперед
        else if(yAxis >= 0 && yAxis < yThreshold) yAxis = 0;
        else if(yAxis < 0 && yAxis > -yThreshold) yAxis = 0;
        
        if(xAxis > 0) {		// Если влево, то тормозим левый мотор
        	motorRight = yAxis;
        ...
        if(motorLeft > 0) {			// Если наклон назад
        	directionL = "-";
        }      
        if(motorRight > 0) {		// Если наклон назад
        	directionR = "-";
        }
